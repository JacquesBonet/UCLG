	...
	function getListQuery()
	{
		...
		if (is_numeric($categoryId)) {
    		...
    		if ($includeSubcategories) {
				...
				/*
				* modif UCLG for taking also parent category
                * $subQuery->join('INNER', '#__categories as this ON sub.lft > this.lft AND sub.rgt < this.rgt');
    			*/
                $subQuery->join('INNER', '#__categories as this ON ((sub.lft > this.lft AND sub.rgt < this.rgt) OR (sub.id = this.parent_id))');

 //               $where = 'this.id = '.(int) $categoryId . ' AND (sub.alias LIKE \'%africa%\' OR this.alias LIKE \'%africa%\')';
                ...
                if (isset($_SESSION['uclg_section']))
                {
                    if (strcmp( $_SESSION['uclg_section'], "world") != 0)
                    {
                        ...
                        $where .= ' AND (sub.alias LIKE \'%' . $filter . '%\' OR sub.alias LIKE \'%' . $filter . '%\')';  // test sub category contains good section
                    }
                }
