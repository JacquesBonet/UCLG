...
$width = $params->get('slideshow_width', 'auto');
$height = $params->get('slideshow_height', 'auto');
$width = $width == 0 || $width == '' ? 'auto' : $width;
$height = $height == 0 || $height == '' ? 'auto' : $height;

$settings = array();
foreach ((array) $params as $key => $value) {
	if (strpos($key, "slideshow_") === 0) {
		$settings[str_replace('slideshow_', '', $key)] = $value;
	}
}
...
<div class="zoo-gallery" id="<?php echo $id; ?>">
	...
	<?php foreach ($thumbs as $image) : ?>
		<li>
			<?php list($width, $height) = @getimagesize($thumbs[0]['img_file']); ?>
...
<script type="text/x-widgetkit" data-widget="slideshow" data-element="#<?php echo $id; ?>">
...
<script type="text/javascript">
	jQuery(function($) {
		if ($['widgetkit']) {
			$.widgetkit.lazyloaders.slideshow($('#<?php echo $id; ?>'), <?php echo json_encode($settings); ?>);
