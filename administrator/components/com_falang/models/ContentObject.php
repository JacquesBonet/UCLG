...
class ContentObject {
	...
	function bind( $formArray, $prefix="", $suffix="", $tryBind=true, $storeOriginalText=false ) {
		...
		for( $i=0; $i<count($elementTable->Fields); $i++ ) {
			...
			if( isset($formArray[$prefix ."refField_". $fieldName .$suffix]) ) {
				...
				// Handle magic quotes compatability
                                //sbou TODO solution on strip not clean
				if (get_magic_quotes_gpc() && $field->Type !== 'htmltext') {
					$formArray[$prefix ."refField_". $fieldName .$suffix] = $this->stripSlashesRecursive( $formArray[$prefix ."refField_". $fieldName .$suffix] );
					$formArray[$prefix ."origText_". $fieldName .$suffix] = $this->stripSlashesRecursive( $formArray[$prefix ."origText_". $fieldName .$suffix] );
				}
				else {
        ...
        //sbou TODO check how to replace this 
        //JRequest::_stripSlashesRecursive his now protected 
        //hot fix
