...
class JApplication extends JObject
{
	...
	public function route()
	{
        ...
        // modif uclg
        // on restaure la dernière section quittée

        if (strpos( $uri, "/fr/") != FALSE) // africa
      	{
      		$_SESSION[ 'uclg_lang'] = 'fr';
      		setcookie( 'uclg_lang', 'fr',  time()+7*24*60*60, '/', 'uclggold.org');
      	}
      	else
      	if (strpos( $uri, "/es") != FALSE) // asie-pacific
      	{
              $_SESSION[ 'uclg_lang'] = 'es';
              setcookie( 'uclg_lang', 'es',  time()+7*24*60*60, '/', 'uclggold.org');
      	}
        else
        {
       		$_SESSION[ 'uclg_lang'] = 'eng';
       		setcookie( 'uclg_lang', 'eng',  time()+7*24*60*60, '/', 'uclggold.org');
       	}

		if (strpos( $uri, "africa-section") != FALSE) // africa
		{
			$_SESSION[ 'uclg_section'] = 'africa';
			setcookie( 'uclg_section', 'africa',  time()+7*24*60*60, '/', 'uclggold.org');
		}
		else
		if (strpos( $uri, "asia-pacific-section") != FALSE) // asie-pacific
		{
			$_SESSION[ 'uclg_section'] = 'asia-pacific';
			setcookie( 'uclg_section', 'asia-pacific',  time()+7*24*60*60, '/', 'uclggold.org');
		}
		else
        if (strpos( $uri, "hors-ue-section") != FALSE) // hors ue
        {
            $_SESSION[ 'uclg_section'] = 'hors-ue';
            setcookie( 'uclg_section', 'hors-ue',  time()+7*24*60*60, '/', 'uclggold.org');
        }
        else
        if (strpos( $uri, "ue-section") != FALSE) // ue
        {
            $_SESSION[ 'uclg_section'] = 'ue';
            setcookie( 'uclg_section', 'ue',  time()+7*24*60*60, '/', 'uclggold.org');
        }
        else
        if (strpos( $uri, "mewa-section") != FALSE) // mewa
        {
            $_SESSION[ 'uclg_section'] = 'mewa';
            setcookie( 'uclg_section', 'mewa',  time()+7*24*60*60, '/', 'uclggold.org');
        }
        else
        if (strpos( $uri, "eurasia-section") != FALSE) // eurasia
        {
              $_SESSION[ 'uclg_section'] = 'eurasia';
              setcookie( 'uclg_section', 'eurasia',  time()+7*24*60*60, '/', 'uclggold.org');
        }
        else
        if (strpos( $uri, "latin-america-section") != FALSE) // latin america
        {
              $_SESSION[ 'uclg_section'] = 'latin-america';
              setcookie( 'uclg_section', 'latin-america',  time()+7*24*60*60, '/', 'uclggold.org');
        }
        else
        if (strpos( $uri, "north-america-section") != FALSE) // north america
        {
              $_SESSION[ 'uclg_section'] = 'north-america';
              setcookie( 'uclg_section', 'north-america',  time()+7*24*60*60, '/', 'uclggold.org');
        }
        else
        if (strpos( $uri, "world-section")  != FALSE) // Global Home
        {
            $_SESSION[ 'uclg_section'] = 'world';
			setcookie( 'uclg_section', 'world',  time()+7*24*60*60, '/', 'uclggold.org');
		}
		else
        if ( isset($_SESSION[ 'uclg_section']) == FALSE)
		{
			if (isset($_COOKIE['uclg_section']))
            {
				$_SESSION[ 'uclg_section'] = $_COOKIE['uclg_section'];
            }
			else
            {
				$_SESSION[ 'uclg_section'] = 'world';
        ...
        if ($uri == "http://www.uclggold.org/" || $uri == "http://www.uclggold.org/es/" || $uri == "http://www.uclggold.org/fr/")
        {
            header('Location: ' . $uri . 'home/' . $_SESSION[ 'uclg_section'] . '-section');  // pour avoir le bon profil
            exit();
        }
